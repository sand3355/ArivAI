...
const sapDiscoveryService = new SAPDiscoveryService(sapClient, logger);
...
async function getOrCreateSession(sessionId?: string, userToken?: string): Promise<{
    ...
    try {
        ...
        const mcpServer = await createMCPServer(discoveredServices, sapDiscoveryService, { userToken });
    ...
    return new Promise((resolve, reject) => {
        try {
            const server = app.listen(port, async () => {
                ...
                // Discover SAP OData services (V2: lightweight catalog, no metadata)
                logger.info('🔍 Discovering SAP OData services...');
                discoveredServices = await sapDiscoveryService.discoverCatalog();
